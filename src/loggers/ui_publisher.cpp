...
{
//------------------------------------------------------
std::mutex UIPublisher::used_ports_mutex;
std::set<unsigned> UIPublisher::used_ports;
...
struct UIPublisher::PImpl
{
  PImpl() : context(), server(context, ZMQ_REP), publisher(context, ZMQ_PUB)
  {
    ...
    recording_fist_time = std::chrono::duration_cast<std::chrono::microseconds>(
...
UIPublisher::UIPublisher(const BT::Tree& tree,
  ...
  _p(new PImpl())
{
  ...
  {
    std::unique_lock<std::mutex> lk(UIPublisher::used_ports_mutex);
    if(UIPublisher::used_ports.count(server_port) != 0 ||
        UIPublisher::used_ports.count(server_port+1) != 0)
    {
      auto msg = StrCat("Another instance of UIPublisher is using port ",
                        std::to_string(server_port));
      throw LogicError(msg);
    }
    UIPublisher::used_ports.insert(server_port);
    UIPublisher::used_ports.insert(server_port+1);
  }
  ...
  _p->server_thread = std::thread(&UIPublisher::serverLoop, this);
  _p->heartbeat_thread = std::thread(&UIPublisher::heartbeatLoop, this);
}

void UIPublisher::setMaxHeartbeatDelay(std::chrono::milliseconds delay)
{
  _p->max_heartbeat_delay = delay;
}

std::chrono::milliseconds UIPublisher::maxHeartbeatDelay() const
{
...
UIPublisher::~UIPublisher()
{
  ...
  {
    std::unique_lock<std::mutex> lk(UIPublisher::used_ports_mutex);
    UIPublisher::used_ports.erase(_p->server_port);
    UIPublisher::used_ports.erase(_p->server_port+1);
  }
}

void UIPublisher::callback(Duration ts, const TreeNode& node,
                               NodeStatus prev_status, NodeStatus new_status)
{
  ...
  *(_p->status_buffermap.at(node.UID())) = status;
  ...
  if(_p->recording)
  {
    ...
    while(_p->transitions_buffer.size() > 10240) {
      _p->transitions_buffer.pop_front();
    }
  }
}

void UIPublisher::flush()
{
  // nothing to do here...
}

void UIPublisher::serverLoop()
{
...
void BT::UIPublisher::enableAllHooks(bool enable)
{
  std::unique_lock<std::mutex> lk(_p->hooks_map_mutex);
...
void UIPublisher::heartbeatLoop()
{
...
std::vector<uint8_t>
UIPublisher::generateBlackboardsDump(const std::string &bb_list)
{
...
bool UIPublisher::insertHook(std::shared_ptr<Monitor::Hook> hook)
{
...
bool UIPublisher::unlockBreakpoint(Position pos, uint16_t node_uid, NodeStatus result, bool remove)
{
  auto it = _p->nodes_by_uid.find(node_uid);
  if( it == _p->nodes_by_uid.end())
  {
...
bool UIPublisher::removeHook(Position pos, uint16_t node_uid)
{
  auto it = _p->nodes_by_uid.find(node_uid);
  if( it == _p->nodes_by_uid.end())
  {
...
void UIPublisher::removeAllHooks()
{
...
Monitor::Hook::Ptr UIPublisher::getHook(Position pos, uint16_t node_uid)
{
  auto hooks = pos == Position::PRE ? &_p->pre_hooks : &_p->post_hooks;
  std::unique_lock<std::mutex> lk(_p->hooks_map_mutex);
